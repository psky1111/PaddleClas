...
class PatchEmbed(nn.Layer):
    ...
    def __init__(self,
                 ...
                 mode='pope'):
        super().__init__()
        self.window_size = ((img_size[1] // (2**sub_num)),
...
class SVTRNet(nn.Layer):
    ...
    def forward_features(self, x):
        ...
        x = x + resize_pos_embed(
            self.pos_embed,
            self.patch_embed.window_size,
            output_dimensions,
            num_extra_tokens=0)
        x = self.pos_drop(x)
        for blk in self.blocks1:
            x = blk(x,
                    output_dimensions)  #[batch_size, seq_length, hidden_size]
        if self.patch_merging is not None:
            x, output_dimensions = self.sub_sample1(
                x.transpose([0, 2, 1]).reshape([
                    0, self.embed_dim[0], output_dimensions[0],
                    output_dimensions[1]
                ]))
        ...
        if self.patch_merging is not None:
            x, output_dimensions = self.sub_sample2(
                x.transpose([0, 2, 1]).reshape([
                    0, self.embed_dim[1], output_dimensions[0],
                    output_dimensions[1]
                ]))
